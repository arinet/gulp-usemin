      ...
      if (typeof block == 'string') {
        ...
        pipeline(block.name, block.files, block.tasks, function(name, file) {
          ...
          if (path.extname(file.path) == '.js')
            html[i] += '<script src="' + name.replace(path.basename(name), path.basename(file.path)) +  '"' + createHTMLAttributes(jsAttributes, jsCounter++) + (block.asyncJs ? ' async' : '' ) + (block.deferJs ? ' defer="defer"' : '' ) +'></script>';
          resolve();
