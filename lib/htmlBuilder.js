    ...
    return new Promise(function(resolve) {
      if (typeof block == 'string') {
        ...
        pipeline(block.name, block.files, block.tasks, function(name, file) {
          push(file);
          var jsFilePath = options && options.jsBundlePath ? options.jsBundlePath : name.replace(path.basename(name), path.basename(file.path));
        ...
        pipeline(block.name, block.files, block.tasks, function(name, file) {
          push(file);
          var cssFilePath = options && options.cssBundlePath ? options.cssBundlePath : name.replace(path.basename(name), path.basename(file.path));
