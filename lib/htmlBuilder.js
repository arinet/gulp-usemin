    ...
    return new Promise(function(resolve) {
      if (typeof block == 'string') {
        ...
        pipeline(block.name, block.files, block.tasks, function(name, file) {
          push(file);
          var jsFilePath = options.jsBundlePath ? options.jsBundlePath : name.replace(path.basename(name), path.basename(file.path))
          if (path.extname(file.path) == '.js')
            html[i] = '<script src="' + jsFilePath + '"' +  (options.enableAsyncJs ? ' async' : '' ) + '></script>';
          resolve();
        ...
        pipeline(block.name, block.files, block.tasks, function(name, file) {
          push(file);
          var cssFilePath = options.cssBundlePath ? options.cssBundlePath : name.replace(path.basename(name), path.basename(file.path))
