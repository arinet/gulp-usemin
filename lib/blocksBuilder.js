...
module.exports = function(file, options) {
  ...
  var startReg = /<!--\s*build:(\w+)(?:(?:\(([^\)]+?)\))?\s+(\/?([^\s]+?))?)?\s*(async)?\s*-->/gim;
  ...
  for (var i = 0, l = sections.length; i < l; ++i) {
    if (sections[i].match(startReg)) {
      ...
      if (startCondLine && endCondLine)
		...
		if (section[5]){
			options.enableAsyncJs = true;
		}
		
      if (section[1] !== 'remove') {
        if (jsReg.test(section[6])) {
          blocks.push({
            type: section[1].indexOf('inline') !== -1 ? 'inlinejs' : 'js',
            nameInHTML: options.jsBundlePath ? options.jsBundlePath : section[3],
            name: options.outputJsPath ? outputPath + options.outputJsPath : outputPath + section[4],
          ...
          blocks.push({
            ...
            nameInHTML: options.cssBundlePath ? options.cssBundlePath : section[3],
            name: options.outputCssPath ? outputPath + options.outputCssPath: outputPath + section[4],
