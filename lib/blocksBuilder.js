...
module.exports = function(file, options) {
  ...
  function modifySectionsBasedOnSpecifiedOptions(section, bundleOption, pathOption, blkCnt){
    if(options && bundleOption){ 
		...
		bundleOption = bundleOption.replace(bundleExtension, "_" + blkCnt > 0 ? blkCnt : "" + bundleExtension);
		section[3] = bundleOption;
		section[4] = bundleOption;
	}
	if(options && pathOption){
		var pathExtension = path.extname(pathOption);
		pathOption = pathOption.replace(pathExtension, "_" + blkCnt > 0 ? blkCnt : "" +  pathExtension);
    ...
    if (buildBlock) {
      ...
      if (section[1] !== 'remove') {
        if (jsReg.test(section[6])) {
		  ...
		  modifySectionsBasedOnSpecifiedOptions(section, options.cssBundlePath, options.outputCssPath,i);
          blocks.push({
